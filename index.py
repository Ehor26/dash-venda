...
df_data = pd.read_csv(r"assets\\supermarket_sales.csv")
...
app.layout = html.Div(children=[
                dbc.Row([
                    dbc.Col([
                        ...
                        html.H5("Varial de análise:", style={"margin-top": "30px"}),
...
def render_graphs(cities, main_variable):
    ...
    df_city = df_filtro.groupby(["City"])[main_variable].apply(operation).to_frame().reset_index()
    df_gender = df_filtro.groupby(["Gender", "City"])[main_variable].apply(operation).to_frame().reset_index()
    df_payment = df_filtro.groupby("Payment")[main_variable].apply(operation).to_frame().reset_index()

    df_income_time = df_filtro.groupby("Date")[main_variable].apply(operation).to_frame().reset_index()
    df_product_income = df_filtro.groupby(["Product line","City"])[main_variable].apply(operation).to_frame().reset_index()
...
#========Graficos=============
    ...
    fig_income_date = px.bar(df_income_time,
                      y=main_variable,
                      x="Date")
